...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      if ((user as any).street) setStreet((user as any).street);
      if ((user as any).city) setCity((user as any).city);
      if ((user as any).postalCode) setPostal((user as any).postalCode);
    }
  }, [user]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (phone && !phoneRegex.test(phone.trim())) {
        toast.error('Số điện thoại không hợp lệ (phải bắt đầu bằng 0 và có đúng 10 số)');
        setIsSaving(false);
        return;
      }
      ...
      const { data } = await api.put(`/users/${user.id}/profile`, payload);
      updateUser(data);
      
      toast.success('Your profile has been updated successfully! ✨');
      setIsEditing(false);
    } catch (err: any) {
      toast.error(err.response?.data || 'Có lỗi xảy ra khi cập nhật thông tin.');
    } finally {
      setIsSaving(false);
    }
  };
          ...
          <div className="bg-white rounded-[2rem] p-8 md:p-10 shadow-sm border border-gray-100 transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 mb-12">
              ...
              <div>
                ...
                {isEditing ? (
                   <input type="email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full bg-[#f4f8f7] border-none rounded-2xl px-5 py-3.5 text-sm focus:ring-2 focus:ring-[#78ad44] outline-none text-gray-900 font-bold" />
                ) : (
            ...
            {isEditing && (
              <div className="flex justify-end gap-3 pt-8 mt-8 border-t border-gray-100 animate-in slide-in-from-bottom-2 duration-300">
                ...
                <button 
                  onClick={handleSave}
                  disabled={isSaving}
                  className="bg-[#212529] hover:bg-[#111] text-white px-8 py-4 rounded-2xl font-bold transition-all shadow-lg flex items-center gap-2 active:scale-95 disabled:bg-gray-400 disabled:cursor-not-allowed"
                >
