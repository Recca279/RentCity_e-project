...
export default function AuthModal() {
  ...
  // Biến error bây giờ CHẮC CHẮN phải là string
  ...
  // Hàm trích xuất lỗi an toàn (Chống sập trang)
  const getSafeErrorMessage = (err: any, defaultMsg: string) => {
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(getSafeErrorMessage(err, 'Email hoặc mật khẩu không đúng. Thử lại nhé!'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleRegister = async (e: FormEvent) => {
    e.preventDefault();
    if (!regName || !regEmail || !regPhone || !regPassword) { setError('Vui lòng điền đầy đủ thông tin.'); return; }
    if (regPassword !== regConfirm) { setError('Mật khẩu xác nhận không khớp.'); return; }

    const phoneRegex = /^0\d{9}$/;
    if (!phoneRegex.test(regPhone.trim())) {
      setError('Số điện thoại không hợp lệ (phải bắt đầu bằng 0 và gồm đúng 10 chữ số).');
      return;
    }
    ...
    try {
      ...
      setIsLogin(true); // Đăng ký xong tự nhảy sang tab Login
      setLoginEmail(regEmail); // Điền sẵn email cho tiện
    } catch (err: any) {
      setError(getSafeErrorMessage(err, 'Đăng ký thất bại, email hoặc sđt có thể đã được sử dụng.'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className={`bg-white w-full max-w-[900px] md:min-h-[600px] rounded-2xl shadow-2xl flex flex-col md:flex-row relative overflow-hidden animate-in zoom-in-95 duration-300 ${!isLogin ? 'md:flex-row-reverse' : ''}`}>
        ...
        <button onClick={closeAuthModal} className="absolute top-4 right-4 z-50 text-gray-400 hover:text-gray-700 bg-white/50 backdrop-blur-md hover:bg-gray-100 rounded-full p-2 transition-all">
          <svg width="18" height="18" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M13 1L1 13M1 1l12 12" /></svg>
        </button>
          ...
          {isLogin ? (
            <form onSubmit={handleLogin} className="w-full space-y-3">
              <div className="relative">
                ...
                <input type="email" placeholder="Email" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} className="w-full bg-[#f4f8f7] border-none rounded-md px-12 py-3.5 text-sm focus:ring-1 focus:ring-[#49B096] outline-none text-gray-700" />
              </div>
              <div className="relative">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={18} />
                <input type={showPassword ? 'text' : 'password'} placeholder="Password" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} className="w-full bg-[#f4f8f7] border-none rounded-md px-12 py-3.5 text-sm focus:ring-1 focus:ring-[#49B096] outline-none text-gray-700 pr-12" />
              ...
              <div className="text-[11px] text-gray-400 bg-gray-50 rounded-xl p-3 leading-relaxed">
                <span className="font-bold text-gray-500">Notice:</span><br />
                Hệ thống đang kết nối với Database thật. Vui lòng nhập đúng tài khoản đã đăng ký!
              </div>
            ...
            <form onSubmit={handleRegister} className="w-full space-y-3 max-h-[360px] overflow-y-auto pr-1">
              ...
              <input type="tel" placeholder="Phone number (Bắt buộc)" value={regPhone} onChange={e => setRegPhone(e.target.value)} className="w-full bg-[#f4f8f7] border-none rounded-md px-4 py-3.5 text-sm focus:ring-1 focus:ring-[#49B096] outline-none text-gray-700" />
              <div className="relative">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={18} />
